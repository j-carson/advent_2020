...
class Tile:
    ...
    def trim_all_edges(self):
        return (self.trim_edge(Edge.North)
    ...
    def count_pattern(self, pattern):
        ...
        for row in range(walk_rows):
            for col in range(walk_cols):
                if (other & me[row:row+other_rows,col:col+other_cols] == other).all():
                    count += 1
        return count
...
class TilePuzzle:
    ...
    def possible_neighbors(self):
        ...
        for neighbor_set in reverse_map.values():
            for neighbor in neighbor_set:
                tile_to_neighbors[neighbor] = \
                    tile_to_neighbors[neighbor].union(neighbor_set - set([neighbor]))
    ...
    def assemble_tiles(self):
        ...
        for col in range(2, ncol-1):
            prev_coord = (0, col-1)
            tile_id = [e for e in neighs[self.layout[prev_coord]] 
                          if e in edges][0]
        ...
        # top right corner
        prev_coord = (0, ncol - 2)
        tile_id = [c for c in neighs[self.layout[prev_coord]] 
                   if c in corners][0]
        ...
        for row in range(1, nrow - 1):
            for col in range(1, ncol -1):
                prev_coord = (row - 1, col)
                tile_id = [i for i in neighs[self.layout[prev_coord]] 
                              if i in interiors][0]
                self.layout[row, col] = tile_id
    ...
    def orient_tiles(self):
        ...
        for col in (0, ncol -1):
            for row in range(1, nrow):
                self[row,col] = self[row-1,col].match_edges(Edge.South, self[row,col])
